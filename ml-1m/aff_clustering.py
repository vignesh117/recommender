...
def getDistance(a, b):
	...
	return  1 - (np.dot(a,b)/(math.sqrt(np.dot(a,a)) * math.sqrt(np.dot(b,b))))
...
def calculateCentroid(points):
	#print points
	#print 'I was called'
	centroids = [0 for i in range(len(points) - 1)]
	for p in points:
		act_vals = p[1:]
		for i in range(len(centroids)):
			centroids[i]+=act_vals[i]
	centroids = map(lambda x:x/(len(points) - 1),centroids)
...
# initally randomly selected k points will be centroids
	...
	while count <=m :
		...
		for p in points:
			pv = p.value
			if count != 0 and old_cluster == None:
				#print point_dict
				oc = point_dict

			elif count ==0 and old_cluster == None:
				oc = -1

			else:
				oc = old_cluster[pv[0]]
			dist = getDistance(pv[1:],calculateCentroid(clusters[0]))
			#print dist
			index = 0
...
def validate_constraints(cluster, threshold,p,oc,arm):
	...
	pv = p.value
	affinity = p.affinity
	max_affinity = max(affinity.values())
